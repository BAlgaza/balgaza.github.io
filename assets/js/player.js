window.addEventListener('message', async e => {
  // Objetivo para probar el Reproductor SOLO en localhost
  ...
  console.log('[CR Premium] Pemutar ditemukan ganz');

  // Variables principales
  const r = { 0: '720', 1: '1080', 2: '480', 3: '360', 4: '240' };
  const lgLangs = { 'idID': 'Indonesia (ID)', 'enUS': 'English (US)', 'enGB': 'English (UK)', 'esLA': 'Español (LA)', 'esES': 'Español (ES)', 'ptPT': 'Português (PT)', 'frFR': 'Français (FR)', 'deDE': 'Deutsch (DE)', 'arME': '(ME) عربي', 'itIT': 'Italiano (IT)', 'ruRU': 'Русский (RU)' };
  const epLangs = { 'idID': 'Episode', 'enUS': 'Episode', 'enGB': 'Episode', 'esLA': 'Episodio', 'esES': 'Episodio', 'ptPT': 'Episódio', 'frFR': 'Épisode', 'deDE': 'Folge', 'arME': 'الحلقة', 'itIT': 'Episodio', 'ruRU': 'Серия' };
  const fnLangs = { 'idID': 'FINAL', 'enUS': 'FINAL', 'enGB': 'FINAL', 'esLA': 'FINAL', 'esES': 'FINAL', 'ptPT': 'FINAL', 'frFR': 'FINALE', 'deDE': 'FINALE', 'arME': 'نهائي', 'itIT': 'FINALE', 'ruRU': 'ФИНАЛЬНЫЙ' };
  ...
  if (force_mp4) console.log('[CR Premium] PAKSA MP4 (Chromecast solution)');

  // Obtener transmisiones
  const streamlist = getStreams(video_config_media['streams']);

  if (!streamlist || streamlist.length === 0) {
      displayError(`Video tidak ditemukkan.`);
      return;
  }
  ...
  // Pistas populares y reproductor de carga
  ...
  (() => {
      ...
      function download_ButtonClickAction() {
          ...
          if (!didDownload) {
              didDownload = true;
              console.log('[CR Premium] Mengunduh font:');
      ...
      jwplayer()
          .on('ready', () => {
              // Establezca la hora del video profesional guardado en localStorage
              if (localStorage.getItem(video_id) != null) {
                  const t = localStorage.getItem(video_id);
                  document.getElementsByTagName('video')[0].currentTime = t >= 5 ? t - 5 : t;
              }
              // Mantiene la pantalla completa + reproducción automática si se ha redirigido usando la función "Siguiente"/"Siguiente"
          ...
          .on('error', e => {
              displayError(`Informasi lebih lanjut di Konsol.\n${linkIssue(`Código: ${e.code}`)}`);
              console.error(e);
          })
      ...
      // Ahorra tiempo de video cada 7 segundos.
      setInterval(() => {
  ...
  // MP4 (descargar) - Gratis: Obtener enlace directo por defecto
  ...
  // Comprueba si existe la URL video_mp4_array[lang][id] y calcula el tamaño de descarga
      ...
      http.onreadystatechange = () => {
          if (http.readyState == 4 && http.status == 200) {
              ...
              if (!fileSize) return setTimeout(() => linkDownload(id), 5000);
                  ...
                  return console.log(`[CR Premium] Sumber yang ditambahkan: ${r[id]} (${return_fileSize})`);
              }
  ...
  function getLocalEpisodeTitle() {
      const episode_translate = `${epLangs[user_lang[0]] ? epLangs[user_lang[0]] : 'Episodio'} `;
  ...
  function updateWebVideoCasterAnchor() {
      ...
      console.log(`[CR Premium] Dia mendefinisikan WVC sebagai a ${r[quality]}p`);
  ...
  function getSourceLocale() {
      try {
          ...
          displayError(`Las cookies ${linkIssue('mereka dinonaktifkan')}!</code>`);
          throw err;
      }
  }

  function displayError(info) {
      const msg = 'Kesalagan Breee astaga (>﹏<)\n' + info;
